...
MODEL_PATH = "D:\\Personal Projects\\Basketball Project\\YOLO Results\\U-net best model\\best_model_epoch89.pth"
...
def apply_colormap(segmented_mask):
    """Apply colormap to segmentation mask for better visualization"""
    if NUM_CLASSES == 1:
        # Binary segmentation - use jet colormap
        colored_mask = cv2.applyColorMap(segmented_mask, cv2.COLORMAP_JET)
        return Image.fromarray(cv2.cvtColor(colored_mask, cv2.COLOR_BGR2RGB))
    else:
        # Multi-class segmentation
        # Create a colormap for each class
        cmap = plt.cm.get_cmap('viridis', NUM_CLASSES)
        colored_mask = cmap(segmented_mask / 255)[:, :, :3]
        return Image.fromarray((colored_mask * 255).astype(np.uint8))

# Then modify the perform_segmentation return:
segmented_pil = apply_colormap(segmented_mask)
...
def process_uploaded_image(uploaded_file):
    ...
    try:
        ...
        with st.spinner('Performing segmentation...'):
            segmented_result = perform_segmentation(image_tensor, segmentation_model)
